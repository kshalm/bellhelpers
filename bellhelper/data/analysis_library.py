...
def calcSettings(det, key, syncArray, ch0=2, ch1=4):
    ...
    return({'settings': settings})  # , 'setting0Bool': setting0Bool, 'setting1Bool': setting1Bool}
...
def calc_period(det, divider=800, syncCh=6):
    ...
    laser_period2[errorLP] = laser_period2[np.roll(errorLP, -1)] #Replace the errant periods with the previous period.
...
def check_for_timetagger_jump_single_party(rawData, paramsCh, ch='sync'):
    ...
    diffTTags = np.abs(np.diff(chTTAG))
    avgDiff = np.mean(diffTTags)
    scale = 2
    if ch=='detector':
        scale =500
...
def calc_data_properties(data, paramsCh, findingOffset = False, findPk = False):
    ...
    for key in reducedData:
        ...
        try:
            res = calcSettings(det, key, syncArray, ch['setting0'], ch['setting1'])
        ...
        phase, phaseHist, pk, laserPulse, ttagModSync, laserPeriod = calc_phase(det, divider, ch)
        ...
        n,x = np.histogram(laserPulse[inWindowBool&clickBool], np.arange((divider+1)))
        ...
        results[key]['laserPulsePC'] = laserPulse[inWindowBool&clickBool&inPCWindowBool]
        results[key]['laserPulseNotPC'] = laserPulse[inWindowBool&clickBool&outPCWindowBool]
        ...
        syncLaserPulse = np.diff(((clickBool&inWindowBool).cumsum())[syncBool])
        ...
        #
        laserPulse2 = laserPulse + (divider)*(syncArray)  # effectively sub-trial #
        results[key]['laserPulse2'] = laserPulse2* (inWindowBool&clickBool).astype(int)
        # print("LASER PULSE", key, laserPulse)
        # print(key, "Nearly done")
        if findingOffset:
            # print(key, "findingOffset")
            n,x = np.histogram(laserPulse2[inWindowBool], np.arange((divider+1)*(nSyncs+1)))
            results[key]['laserPulseHist2']={'x': x[:-1], 'y': n, 'name':key}
...
def calc_pc_on_off_slots(rawData, paramsCh):
    ...
    # print(offset)
    pcStartStop = {'alice': [], 'bob': [], 'bbOffset': offset, 'bb':{'alice':[], 'bob':[]}}
...
def calc_pc_on_off_slots_bbone(rawData, paramsCh):
    ...
    abdelay = 0
    overlapWindow, eventNumber, numSyncs, startSync, stopSync, counts = calc_windows(results, reducedData, offsetLaserPulse, paramsCh)
    ...
    for key in ar:
        ...
        bSyncTT = reducedData[key]['ttag'][bTrialBool] # only the time tags associated with the bSync
        # bSyncTTags = bSyncTT - bSyncTT[0]
        laserPeriod = np.mean(results[key]['laserPeriod'])
        bPulseNumber = (np.round(bSyncTTags*1./laserPeriod)).astype(int) # Divide by the laser period (in ttags) to find the pulse number

        # Now look at the detector time tags and figure out which trial they belong to.
        detBool = (overlapWindow[key]['eventBool'] >= firstBsync) # All the locations where a detection event occured within the coinc radius.
        detAllBool = reducedData[key]['ch'] == ch['detector']
        detTTags = reducedData[key]['ttag']# all of the time tags of valid detection events
        detTTagsModbSync = (detTTags - bSyncTT[bTrialCum])[detBool]
        detPulseNumber = (np.floor(detTTagsModbSync*1./laserPeriod)).astype(int)
        # laserPulse = np.floor(ttagModSync/laser_period2[syncArray])
...
def calc_offset(rawData, paramsCh):
    ...
    results, reducedData = calc_data_properties(rawData, paramsCh, findingOffset = True, findPk = True)
    ...
    if len(coinc)>0:
        if abdelay < 0:
            ...
            bIndx = np.in1d( results['bob']['laserPulse2'].astype(int), coinc - corr)
        else:
            aIndx = np.in1d( results['alice']['laserPulse2'].astype(int), coinc + corr)
        ...
        print( len(coinc), ttagOffset, ttagOffset - min(diffTTag), ttagOffset - (max(diffTTag)) )
    else:
...
def trim_data(data, ttagOffset, paramsCh, nSyncs = None):
    ...
    bData = data['bob']['ttag']*1. + ttagOffset*1. # event number for all bob tags, corrected for delay
    ...
    if startTTagA[0] < startTTagB[0] :
        indxB = 0
        if np.abs(startTTagA[0]- startTTagB[0])<np.abs(startTTagA[1]- startTTagB[0]):
            indxA = 0
        else:
            indxA = 1
    if startTTagA[0] > startTTagB[0] :
        indxA = 0
        if np.abs(startTTagA[0]- startTTagB[0])<np.abs(startTTagA[0]- startTTagB[1]):
            indxB = 0
        else:
            indxB = 1
    ...
    if nSyncs != None:
        ...
        if stopIndx != nSyncs:
            ...
            stopTTag = max(startTTagA[nSyncs-1 + indxA], startTTagB[nSyncs-1+ indxB])
    ...
    trimmedData['alice']['ttag'] = trimmedData['alice']['ttag'] - trimmedData['alice']['ttag'][0]#.astype('float')
    trimmedData['bob'] =  data['bob'][bClicks]
    trimmedData['bob']['ttag'] = trimmedData['bob']['ttag'] - trimmedData['bob']['ttag'][0]#rescaledB#.astype('float')
...
# @jit
def calc_windows(results, reducedData, offsetLaserPulse, paramsCh, notPC = False):
    ...
    for key in results:
        ...
        overlapWindow[key]['syncTags'] = reducedData[key][overlapWindow[key]['bool'] & syncBool]
        eventBool = overlapWindow[key]['bool'] & results[key]['clickBool'] & results[key]['inWindowBool']
        ...
        # print()
        eventSyncNumber = results[key]['syncArray']+delay-startSync  #  Need to subtract startSync if >0
        if notPC:
            eventNumber[key] = (results[key]['laserPulseNotPC'] + 1*(eventSyncNumber-1) * int(results[key]['divder']) +1* offsetLaserPulse[key]).astype(int)
        else:
            eventNumber[key] = (results[key]['laserPulse'] + 1*(eventSyncNumber-1) * int(results[key]['divder']) +1* offsetLaserPulse[key]).astype(int)
        overlapWindow[key]['eventBool'] = eventBool
    ...
    # print('')
    coinEventNum = np.intersect1d(eventNumber['alice'][overlapWindow['alice']['eventBool']],eventNumber['bob'][overlapWindow['bob']['eventBool']])
...
# @jit
def get_stats(results, reducedData, offsetLaserPulse, abdelay, pockelProp, paramsCh, overlapWindow = None, numSyncs = None, PC = False):
    ...
    if (overlapWindow == None) | (numSyncs == None):
        overlapWindow, eventNumber, numSyncs, startSync, stopSync, counts = calc_windows(results, reducedData, offsetLaserPulse, paramsCh)
    ...
    if PC:
        for key in results:
            ...
            laserPulse = results[key]['laserPulse']#[overlapWindow[key]['eventBool']]

            pockelBool = (laserPulse >= pcStart) & (laserPulse <= (pcStart+pockelProp['length']))
            PCeventBool = overlapWindow[key]['eventBool'] & pockelBool

            valTest = 2**( (laserPulse * PCeventBool.astype(int)) - pcStart* PCeventBool.astype(int) )
            ...
            pcOverlapWindow[key]['settings'] = np.delete(overlapWindow[key]['settings'], -1)

            pcEventNumber[key] = pcOverlapWindow[key]['eventCode']>0

        coinEventNumPC = np.intersect1d(np.nonzero(pcEventNumber['alice'])[0], np.nonzero(pcEventNumber['bob'])[0])
        totalSettings = pcOverlapWindow['alice']['settings'] + 4*pcOverlapWindow['bob']['settings']
